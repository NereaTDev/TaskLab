...
<div class="max-w-3xl mx-auto py-8 px-4">
    <h1 class="text-2xl font-semibold text-slate-900 mb-4">New task</h1>
    ...
    <form method="POST" action="{{ route('tasks.store') }}" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <input type="text" name="url" value="{{ old('url') }}" class="w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm">
                @error('url')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div>
            ...
            <textarea name="description" rows="8" class="w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" placeholder="Explain what you need. Include context, steps to reproduce, expected behavior, etc.">{{ old('description') }}</textarea>
            @error('description')
                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
            @enderror
        </div>
