...
| Carpeta | Contenido |
|---------|-----------|
| `docs/second-brain/` | Núcleo estratégico: visión, problema, cliente ideal, propuesta de valor, principios (`01-vision.md` a `05-principios.md`), más `frases-clave.md` |
...
| `docs/sprints/` | Bitácora de sprints del Second Brain (objetivo, resultados, pendientes): `Sprint-00.md` a `Sprint-04.md` |
| `docs/modelo/` | Modelos formales de procesos observados (p.ej. el proceso mental del desarrollador), distintos del modelo de dominio en `docs/producto/` |

**Idea clave a recordar en todo momento:** TaskLab no debe tratarse como "un gestor de tareas más". A partir del Sprint 01 se define como **un sistema para preservar el contexto y la trazabilidad de las decisiones que originan el desarrollo de software** (ver `docs/decisiones/D-002-el-problema-es-la-perdida-de-contexto.md`), a partir del Sprint 02 aspira a ser la **memoria operativa del desarrollo de software** (ver `docs/decisiones/D-003-el-tiempo-mas-caro-es-el-de-comprension.md`), a partir del Sprint 03 incorpora **Ready to Understand** como concepto central (ver `docs/producto/ready-to-understand.md` y `docs/decisiones/D-004-ready-to-understand-como-concepto-central.md`), y a partir del Sprint 04 el eje pasa de "comprensión" (estado mental no observable) a **incertidumbre** (evaluable a partir de la información disponible y las preguntas relevantes pendientes): TaskLab no busca generar más documentación, busca reducir la incertidumbre necesaria para comenzar una implementación (ver `docs/producto/incertidumbre.md` y `docs/decisiones/D-005-incertidumbre-como-metrica.md`). No compite inicialmente con Jira/Linear/Shortcut/Trello/ClickUp — se integra con ellos (ver `docs/decisiones/001-no-competir-con-jira.md` y `docs/decisiones/D-001-no-competir-con-jira.md`). Cualquier sugerencia de producto o arquitectura debe respetar este posicionamiento salvo indicación expresa en contra.

**Estado del Second Brain:** Sprints 00 a 04 completados (ver `docs/sprints/`). El problema documentado en `docs/second-brain/02-problema.md` sigue siendo una **hipótesis**, validada externamente con equipos de desarrollo (`docs/investigacion/validacion-externa.md`) y respaldada por los casos reales (`docs/casos/`), los patrones detectados (`docs/investigacion/patrones.md`, P-001 a P-013) y el modelo del proceso mental del desarrollador (`docs/modelo/proceso-mental-del-desarrollador.md`). Hipótesis pendientes de validar: `docs/validaciones/H-001-expediente-completo.md`, `docs/validaciones/H-002-tiempo-de-comprension.md`, `docs/validaciones/H-003-ready-to-understand.md`, `docs/validaciones/H-004-incertidumbre.md`. Existe un primer modelo de dominio provisional en `docs/producto/modelo-del-dominio.md` y un registro de frases clave por sprint en `docs/second-brain/frases-clave.md`. Próximo sprint: diseñar el primer flujo funcional del MVP.
