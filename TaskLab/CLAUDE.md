...
| Carpeta | Contenido |
|---------|-----------|
| `docs/second-brain/` | Núcleo estratégico: visión, problema, cliente ideal, propuesta de valor, principios (`01-vision.md` a `05-principios.md`), más `frases-clave.md`, `preguntas-abiertas.md` y `principios.md` (principios adicionales desde Sprint 06) |
| `docs/producto/` | Definiciones funcionales de producto |
| `docs/investigacion/` | Investigación de mercado, competencia y usuarios, incluyendo `mercado-decisiones.md`, `errores-de-decision.md` y `consecuencias-de-decisiones-ausentes.md` (Sprint 07) |
...
| `docs/arquitectura/` | Arquitectura técnica estratégica, incluyendo la auditoría del repositorio y el diseño del primer flujo del Motor de Comprensión (Sprint 08) |
...
| `docs/sprints/` | Bitácora de sprints del Second Brain (objetivo, resultados, pendientes): `Sprint-00.md` a `Sprint-08.md` (Sprint 08 en progreso) |
| `docs/modelo/` | Modelos formales de procesos observados: `proceso-mental-del-desarrollador.md`, `motor-de-comprension.md`, `contextos.md`, `cadena-de-confianza.md`, `transformacion-de-tasklab.md`, `contexto-como-decisiones.md`. Distinto del modelo de dominio en `docs/producto/` |
| `docs/dominio/` | Objetos y conceptos del dominio de TaskLab: `necesidad.md` (objeto raíz), `decision.md`, `cadena-de-decisiones.md`, `propiedad-de-la-decision.md` |

**Idea clave a recordar en todo momento:** TaskLab no debe tratarse como "un gestor de tareas más". A partir del Sprint 01 se define como **un sistema para preservar el contexto y la trazabilidad de las decisiones que originan el desarrollo de software** (ver `docs/decisiones/D-002-el-problema-es-la-perdida-de-contexto.md`), a partir del Sprint 02 aspira a ser la **memoria operativa del desarrollo de software** (ver `docs/decisiones/D-003-el-tiempo-mas-caro-es-el-de-comprension.md`), a partir del Sprint 03 incorpora **Ready to Understand** como concepto central (ver `docs/producto/ready-to-understand.md` y `docs/decisiones/D-004-ready-to-understand-como-concepto-central.md`), a partir del Sprint 04 el eje pasa de "comprensión" (estado mental no observable) a **incertidumbre** (evaluable a partir de la información disponible y las preguntas relevantes pendientes) (ver `docs/producto/incertidumbre.md` y `docs/decisiones/D-005-incertidumbre-como-metrica.md`), a partir del Sprint 05 el producto se diseña alrededor del **razonamiento del desarrollador** (el "Motor de Comprensión": dónde está, cómo funciona, qué debe cambiar, quién lo conoce, quién lo valida) y no alrededor de la estructura de herramientas externas (ver `docs/modelo/motor-de-comprension.md` y `docs/decisiones/D-006-modelo-mental-del-desarrollador.md`), a partir del Sprint 06 TaskLab parte de **Necesidades**, no de tickets: el objeto raíz de su dominio es la Necesidad, que se transforma en una **cadena de decisiones** explícitas, atribuibles y trazables (ver `docs/dominio/necesidad.md`, `docs/decisiones/D-007-necesidad-como-objeto-raiz.md` y `docs/decisiones/D-008-decisiones-como-unidades-trazables.md`), el Sprint 07 confirma con investigación de mercado (Jira, Linear, Shortcut, Notion, Confluence, PRD, ADR) que ninguna herramienta existente descubre sistemáticamente decisiones implícitas antes de ejecutar: TaskLab **no competirá como gestor de tareas ni como gestor documental**, su posicionamiento es ayudar a descubrir las decisiones necesarias antes de ejecutar trabajo (ver `docs/decisiones/D-009-posicionamiento.md`), y el Sprint 08 comienza a **construir sobre el repositorio Laravel existente** (no uno nuevo): se ha auditado la arquitectura actual y diseñado el primer flujo vertical del Motor de Comprensión, que empezará con una implementación **simulada** intercambiable por una real vía un contrato (interfaz), sin tocar el dominio `Task` existente (ver `docs/arquitectura/auditoria-repositorio-sprint-08.md`, `docs/arquitectura/primer-flujo-motor-comprension.md`, `docs/decisiones/D-010-adaptar-repositorio-laravel-existente.md` y `docs/decisiones/D-011-primer-motor-simulado.md`). No compite inicialmente con Jira/Linear/Shortcut/Trello/ClickUp — se integra con ellos (ver `docs/decisiones/001-no-competir-con-jira.md` y `docs/decisiones/D-001-no-competir-con-jira.md`). Cualquier sugerencia de producto o arquitectura debe respetar este posicionamiento salvo indicación expresa en contra.

**Estado del Second Brain:** Sprints 00 a 07 completados, Sprint 08 en progreso (ver `docs/sprints/`). El problema documentado en `docs/second-brain/02-problema.md` sigue siendo una **hipótesis**, validada externamente con equipos de desarrollo (`docs/investigacion/validacion-externa.md`) y respaldada por los casos reales (`docs/casos/`), los patrones detectados (`docs/investigacion/patrones.md`, P-001 a P-022), el modelo de dominio Necesidad → Decisiones → Ejecución → Validación (`docs/dominio/`) y la investigación de mercado (`docs/investigacion/mercado-decisiones.md`). Se distinguen además error de decisión y error de implementación (`docs/investigacion/errores-de-decision.md`) y cuatro consecuencias de una decisión ausente: bloqueo, trabajo atrapado, implementación incorrecta y abandono (`docs/investigacion/consecuencias-de-decisiones-ausentes.md`). Hipótesis pendientes de validar: `docs/validaciones/H-001-expediente-completo.md` a `docs/validaciones/H-008-descubrimiento-de-decisiones.md`. Preguntas abiertas sin resolver en `docs/second-brain/preguntas-abiertas.md`. Principios adicionales de los Sprints 06-08 en `docs/second-brain/principios.md` (complementa a `docs/second-brain/05-principios.md`). Existe un primer modelo de dominio provisional en `docs/producto/modelo-del-dominio.md` y un registro de frases clave por sprint en `docs/second-brain/frases-clave.md`. **El código de la aplicación aún no se ha modificado para el Sprint 08** — la implementación del primer flujo vertical (Sprint 08B) sigue pendiente.
